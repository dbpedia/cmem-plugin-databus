...
from cmem_plugin_databus.utils import (
    fetch_api_search_result,
    fetch_databus_files,
    fetch_facets_options,
)
...
class DatabusSearch(StringParameterType):
    ...
    autocompletion_depends_on_parameters: list[str] = ["databus_base_url"]  # noqa: RUF012
...
class FacetSearch(StringParameterType):
    ...
    autocompletion_depends_on_parameters: list[str] = ["databus_base_url", "databus_artifact"]  # noqa: RUF012
    ...
    def autocomplete(
        ...
        facet_options = fetch_facets_options(
            databus_base=databus_base_url,
            url_parameters={"type": "artifact", "uri": databus_document},
        )
...
class DatabusFile(StringParameterType):
    ...
    autocompletion_depends_on_parameters: list[str] = [  # noqa: RUF012
        "databus_base_url",
        "databus_artifact",
        "artifact_format",
        "artifact_version",
    ...
    def autocomplete(
        self,
        query_terms: list[str],  # noqa: ARG002
        depend_on_parameter_values: list[Any],
        context: PluginContext,  # noqa: ARG002
    ) -> list[Autocompletion]:
        """Get completion options"""
...
class ResponseStream:
    ...
    def __init__(self, response, chunk_size=1048576):  # noqa: ANN001
        self.response = response
        self.chunk_size = chunk_size

    def _read(self) -> Generator[bytes, None, None]:
        yield from self.response.iter_content(chunk_size=self.chunk_size)
...
@Plugin(
    ...
    " artifact from a DBpedia Databus to a Corporate Memory dataset resource.",
    parameters=[
        PluginParameter(
        ...
        PluginParameter(
            name="target_file",
            label="File",
            description="File name where you want to save the downloaded file from the Databus.",
...
class SimpleDatabusLoadingPlugin(WorkflowPlugin):
    ...
    def __init__(  # noqa: PLR0913
        ...
        databus_artifact: str = "",  # noqa: ARG002 - used to specify the file ID only
        artifact_format: str = "",  # noqa: ARG002 - used to specify the file ID only
        artifact_version: str = "",  # noqa: ARG002 - used to specify the file ID only
        databus_file_id: str = "",
        target_file: str = "",
    ...
    def execute(
        ...
        if databus_file_resp.status_code > http.HTTPStatus.BAD_REQUEST:
        ...
        if upload_response.status_code < http.HTTPStatus.BAD_REQUEST:
            context.report.update(
                ExecutionReport(operation_desc="Download Successful ✓", entity_count=1)
            )
        else:
            context.report.update(
                ExecutionReport(operation_desc="Download Failed ❌", error=upload_response.text)
            )
